...
static long int *MD4Digest(long int *w, int len);
static void setMD4Registers(long int AA, long int BB, long int CC, long int DD);
static long int changeEndianness(long int x);
static void resetMD4Registers(void);
static string stringCat(string first, string second);
static string uint32ToString(long int l);
static long int stringToUint32(string s);
...
#define MD4ROUND2(a,b,c,d,x,s) a += G(b,c,d) + x + (long int)0x5A827999; a = LEFTROTATE(a, s);
#define MD4ROUND3(a,b,c,d,x,s) a += H(b,c,d) + x + (long int)0x6ED9EBA1; a = LEFTROTATE(a, s);

static long int A = 0x67452301;
static long int B = 0xefcdab89;
static long int C = 0x98badcfe;
static long int D = 0x10325476;
...
string uint32ToString(long int l){
...
long int stringToUint32(string s){
	long int l;
	int i;
	l=0;
	for(i=0; i<4; i++){
		l = l|(((long int)((unsigned char)s.c[i]))<<(8*(3-i)));
	}
	return l;
}
...
char *MD4(char *str, int len){
	...
	w = malloc(sizeof(long int)*(m.len/4+2));
...
long int *MD4Digest(long int *w, int len){
	//assumes message.len is a multiple of 64 bytes.
	int i,j;
	long int X[16];
	long int *digest = malloc(sizeof(long int)*4);
...
long int changeEndianness(long int x){
	return ((x & 0xFF) << 24) | ((x & 0xFF00) << 8) | ((x & 0xFF0000) >> 8) | ((x & 0xFF000000) >> 24);
}

void setMD4Registers(long int AA, long int BB, long int CC, long int DD){
	A=AA;
	B=BB;
	C=CC;
	D=DD;
